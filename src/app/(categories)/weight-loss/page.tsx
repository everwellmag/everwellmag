...
export default function DietPlanPage() {
    ...
    return (
        <div className="container mx-auto p-6" style={{ backgroundColor: 'var(--background)', color: 'var(--foreground)' }}>
            {/* Header Section */}
            <div className="relative bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-8 mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {articles.map((article) => {
                    ...
                    return (
                        <div
                            key={article.id}
                            className="group relative bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden min-h-[400px] flex flex-col"
                        >
                            {/* Thumbnail Image */}
                            {thumbnailUrl ? (
                                ...
                                <div className="w-full h-56 bg-gray-200 dark:bg-gray-700 flex items-center justify-center rounded-t-lg">
                            ...
                            <div className="p-6 flex flex-col flex-1">
                                ...
                                {article.author && (
                                    <p className="text-sm mb-4 text-gray-500 dark:text-gray-400 line-clamp-1">
                                ...
                                {article.blocks.map((block, index) => {
                                    if (block.__component === 'shared.rich-text' && 'body' in block && block.body) {
                                        return (
                                            <div key={index} className="mb-4 line-clamp-3 text-sm text-gray-600 dark:text-gray-300">
                                                <ReactMarkdown
                                                    components={{
                                                        p: ({ ...props }) => <p {...props} />,
                                    ...
                                    if (block.__component === 'shared.quote' && 'title' in block && 'body' in block && block.title && block.body) {
                                        return (
                                            <blockquote key={index} className="border-l-4 pl-4 italic mb-4 line-clamp-2" style={{ borderColor: '#3B82F6' }}>
                                                <p className="text-sm text-gray-600 dark:text-gray-300">{block.body}</p>
                                                <p className="text-xs text-gray-500 dark:text-gray-400">— {block.title}</p>
                                            </blockquote>
                                        );
                                    }
                                    return null;
                                ...
                                <div className="mt-auto">
                                    <Link
                                        ...
                                        className="inline-block bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 rounded-md hover:from-blue-600 hover:to-purple-700 transition-colors duration-300"
                                    >
                                        Read More
                                    </Link>
                                </div>
