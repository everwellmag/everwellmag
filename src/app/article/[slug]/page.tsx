...
// Chuẩn type props cho page + metadata
interface PageProps {
    params: {
        slug: string;
    };
...
export async function generateMetadata({ params }: PageProps) {
    ...
    const baseUrl =
        process.env.STRAPI_API_URL || 'https://cms.everwellmag.com';
    const coverImageUrl = article?.cover?.url
        ? toStringSrc(
            `${baseUrl}${isUrlWithPath(article.cover.url, '/uploads')
                ? article.cover.url
                : `/uploads${article.cover.url}`
            }`
        )
        : null;
    ...
    return {
        ...
        description:
            article?.description ||
            `Read more about ${article?.title || 'this article'
            } on Everwell Magazine. Discover expert insights and tips.`,
...
export default async function ArticlePage({ params }: PageProps) {
    ...
    const coverImageUrl = cover?.url
        ? toStringSrc(
            `${baseUrl}${isUrlWithPath(cover.url, '/uploads')
                ? cover.url
                : `/uploads${cover.url}`
            }`
        )
        ...
        switch (block.__component) {
            ...
            case 'shared.quote':
                return (
                    <blockquote
                        key={blockKey}
                        className="border-l-4 pl-4 italic my-6 text-gray-600"
                    >
            ...
            case 'shared.media':
                ...
                return mediaUrl ? (
                    <Image
                        ...
                        src={mediaUrl}
                        alt={
                            block.file?.data?.attributes?.alternativeText || 'Media'
                        }
                    ...
                    <div key={blockKey} className="text-gray-500">
                        [Media: Thiếu dữ liệu]
                    </div>
                );
            case 'shared.slider':
    ...
    const renderedBlocks =
        blocks?.map((block: StrapiBlock, index: number) =>
