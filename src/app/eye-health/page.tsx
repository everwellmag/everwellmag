...
export default function SupplementsForEyeHealthPage() {
        ...
        const fetchArticles = async () => {
            try {
                const response = await fetch(
                    'https://cms.everwellmag.com/api/articles?filters[category][id]=10&pagination[page]=1&pagination[pageSize]=10&populate=*',
                    {
                        headers: {
                ...
                if (!data.data || data.data.length === 0) {
                    throw new Error('No articles found for category ID 10');
                }
    ...
    return (
        <div className="container mx-auto p-4" style={{ backgroundColor: 'var(--background)', color: 'var(--foreground)' }}>
            <h1 className="text-3xl font-bold mb-6" style={{ color: 'var(--foreground)' }}>Supplements for Eye Health</h1>
            <p className="mb-8" style={{ color: 'var(--foreground)' }}>
                Discover supplements to support eye health with expert advice and effective
                solutions.
            </p>
