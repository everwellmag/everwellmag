...
export const CustomMarkdown = ({ content }: MarkdownRendererProps) => {
    return (
        <div className="prose max-w-none" style={{ color: 'var(--text-secondary)', fontFamily: 'Inter, system-ui, sans-serif' }}>
            <ReactMarkdown
                remarkPlugins={[remarkGfm, remarkContentReference]}
                components={{
                    p: ({ children, ...props }: React.ComponentProps<'p'>) => (
                        <p className="mb-4 text-base md:text-lg leading-relaxed" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </p>
                    ),
                    h1: ({ children, ...props }: React.ComponentProps<'h1'>) => (
                        <h1 className="text-3xl font-bold mb-3 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h1>
                    ),
                    h2: ({ children, ...props }: React.ComponentProps<'h2'>) => (
                        <h2 className="text-2xl font-semibold mb-2 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h2>
                    ),
                    h3: ({ children, ...props }: React.ComponentProps<'h3'>) => (
                        <h3 className="text-xl font-semibold mb-2 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h3>
                    ),
                    h4: ({ children, ...props }: React.ComponentProps<'h4'>) => (
                        <h4 className="text-lg font-semibold mb-2 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h4>
                    ),
                    h5: ({ children, ...props }: React.ComponentProps<'h5'>) => (
                        <h5 className="text-base font-semibold mb-2 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h5>
                    ),
                    h6: ({ children, ...props }: React.ComponentProps<'h6'>) => (
                        <h6 className="text-sm font-semibold mb-2 leading-tight" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </h6>
                    ),
                    ul: ({ children, ...props }: React.ComponentProps<'ul'>) => (
                        <ul className="list-disc list-inside mb-3" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </ul>
                    ),
                    ol: ({ children, ...props }: React.ComponentProps<'ol'>) => (
                        <ol className="list-decimal list-inside mb-3" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </ol>
                    ),
                    li: ({ children, ...props }: React.ComponentProps<'li'>) => (
                        <li className="mb-1 text-base md:text-lg leading-relaxed" style={{ color: 'var(--foreground)' }} {...props}>
                            {children}
                        </li>
                    ),
